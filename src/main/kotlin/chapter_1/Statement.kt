...
class Statement(private val invoice: Invoice, private val plays: List<Play>) {
    ...
    fun totalAmount(): Int = invoice.performances.sumOf { amountFor(it) }

    // statement의 volumeCredits 임시변수를 대체
    fun totalVolumeCredits(): Double = invoice.performances.sumOf { volumeCreditsFor(it) }

    fun volumeCreditsFor(perf: Performance) =
        if (playFor(perf).type == "comedy") floor(perf.audience.toDouble() / 5) + maxOf(
            perf.audience - 30,
            0
        ).toDouble()
